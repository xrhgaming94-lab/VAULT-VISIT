...
- Fetches backpack items from multiple endpoints
...
# Multiple GetBackpack URLs
GET_BACKPACK_URLS = [
    "https://client.ind.freefiremobile.com/GetBackpack",
    "https://client.us.freefiremobile.com/GetBackpack",
    "https://clientbp.ggpolarbear.com/GetBackpack"
]
...
def fetch_vault(jwt_token):
    """Fetch backpack items using JWT token from multiple endpoints"""
    all_ids = set()
    errors = []
    ...
    if not all_ids and errors:
        return None, f"All endpoints failed: {'; '.join(errors)}"
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h1 { text-align: center; margin-bottom: 8px; color: #64ffda; font-size: 1.5rem; }
        .subtitle { text-align: center; margin-bottom: 20px; color: #8892b0; font-size: 0.8rem; }
        .form-container {
            background: #112240;
            padding: 15px;
            border-radius: 12px;
        ...
        /* CRITICAL FIX: Phone pe 4 columns */
        .item-grid {
            display: grid;
            gap: 12px;
        ...
        /* Small phone: 3 columns agar 4 bohot chota ho */
...
<body>
    ...
    <div class="filter-bar">
        <div class="filter-group">
            <label>🔍 Search</label>
            <input type="text" id="searchInput" placeholder="Name or ID..." onkeyup="filterItems()">
        </div>
        <div class="filter-group">
            <label>🏷️ Type</label>
                    ...
                    {% for item in items %}
                        <div class="item-card" 
                            ...
                            <img src="https://cdn.jsdelivr.net/gh/ShahGCreator/icon@main/PNG/{{ item.id }}.png" 
                                 onerror="this.src='https://via.placeholder.com/80?text=No+Icon'">
                            <div class="item-name">{{ item.name[:30] }}{% if item.name|length > 30 %}...{% endif %}</div>
...
def index():
    if request.method == 'POST':
        ...
        # Step 2: Fetch vault items using JWT from multiple endpoints
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("Using GetBackpack endpoints:")
    for url in GET_BACKPACK_URLS:
        print(f"  - {url}")
    print("=" * 50)
